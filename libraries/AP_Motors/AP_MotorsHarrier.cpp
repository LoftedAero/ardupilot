...
/*
 *       AP_MotorsHarrier.cpp - ArduCopter motors library
 *       Original AP_MotorsTri Code by RandyMackay. DIYDrones.com
 *       Modified into Harrier frame class by Eric Maglio
 *
 */
#include <AP_HAL/AP_HAL.h>
#include <AP_Math/AP_Math.h>
#include <GCS_MAVLink/GCS.h>
#include "AP_MotorsHarrier.h"

extern const AP_HAL::HAL& hal;

// init
void AP_MotorsHarrier::init(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    if (!SRV_Channels::get_channel_for(SRV_Channel::k_motor7, AP_MOTORS_CH_TRI_YAW)) {
        gcs().send_text(MAV_SEVERITY_ERROR, "MotorsHarrier: unable to setup yaw channel");
    ...
    set_initialised_ok(frame_class == MOTOR_FRAME_HARRIER);
}

// set frame class (i.e. quad, hexa, heli) and type (i.e. x, plus)
void AP_MotorsHarrier::set_frame_class_and_type(motor_frame_class frame_class, motor_frame_type frame_type)
{
    ...
    set_initialised_ok((frame_class == MOTOR_FRAME_HARRIER) && SRV_Channels::function_assigned(SRV_Channel::k_motor7));
}

// set update rate to motors - a value in hertz
void AP_MotorsHarrier::set_update_rate(uint16_t speed_hz)
{
...
void AP_MotorsHarrier::output_to_motors()
{
    switch (_spool_state) {
        ...
        case SpoolState::SPOOLING_DOWN:
            ...
            set_actuator_with_slew(_actuator[5], thrust_to_actuator(_thrust_main));
            rc_write(AP_MOTORS_MOT_1, output_to_pwm(_actuator[1]));
            rc_write(AP_MOTORS_MOT_2, output_to_pwm(_actuator[2]));
            rc_write(AP_MOTORS_MOT_3, output_to_pwm(_actuator[3]));
            rc_write(AP_MOTORS_MOT_4, output_to_pwm(_actuator[4]));
...
uint16_t AP_MotorsHarrier::get_motor_mask()
{
    // Harrier uses channels 1,2,3,4,5, and 7
...
void AP_MotorsHarrier::output_armed_stabilizing()
{
    ...
    if (rpy_low > MIN(_thrust_front, _thrust_rear)) {
        rpy_low = MIN(_thrust_front, _thrust_rear);
    }
...
void AP_MotorsHarrier::output_test_seq(uint8_t motor_seq, int16_t pwm)
{
...
*/
void AP_MotorsHarrier::thrust_compensation(void)
{
 ...
 */
void AP_MotorsHarrier::output_motor_mask(float thrust, uint8_t mask, float rudder_dt)
{
...
float AP_MotorsHarrier::get_roll_factor(uint8_t i)
{
    float ret = 0.0f;
